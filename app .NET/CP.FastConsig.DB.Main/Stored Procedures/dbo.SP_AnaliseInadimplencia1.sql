...
BEGIN
    ...
    select c.IDConsignataria as IDEmpresa, ct.IDConciliacaoGrupo, c.IDFuncionario, c.valordescontado, c.valor, cm.parcela, cm.prazo, ap.IDAverbacao
    into #conciliacao
    from Conciliacao c, ConciliacaoTipo ct, ConciliacaoGrupo cg, ConciliacaoMovimento cm, AverbacaoParcela ap
    ...
    and ct.IDConciliacaoGrupo in (1,2) and c.IDConciliacaoMovimento = cm.IDConciliacaoMovimento and cm.IDAverbacaoParcela = ap.IDAverbacaoParcela
	...
	select c.idempresa,c.IDFuncionario, c.valor, c.parcela, f.idfuncionariosituacao, c.idconciliacaogrupo, c.IDAverbacao, c.Prazo
	into #inadimplente
	...
	select c.idempresa,c.IDFuncionario, c.valor, c.parcela, c.idfuncionariosituacao, c.idaverbacao, c.prazo
	into #recuperavel
	from #inadimplente c
	...
	insert into TmpRecuperavelPorFolha(competencia, idempresa, nome, matricula, cpf, parcela, pagas_prazo, margem, ematraso)
	select @Competencia, r.IDEmpresa, p.Nome, f.Matricula, p.CPF, r.parcela,CONVERT(varchar(2),(r.Parcela - (select COUNT(*) from AverbacaoParcela where IDAverbacao = r.idaverbacao and IDAverbacaoParcelaSituacao = 1 and Competencia <= @Competencia)))+'/'+CONVERT(varchar(2),r.prazo),
	...
	(select SUM(valor) from AverbacaoParcela where IDAverbacao = r.idaverbacao and IDAverbacaoParcelaSituacao = 1 and Competencia <= @Competencia)	
	from #recuperavel r, #func f, Pessoa p
	where r.IDFuncionario = f.IDFuncionario and f.IDPessoa = p.IDPessoa
 
	insert into TmpNaoRecuperavel(competencia, idempresa, nome, matricula, cpf, parcela, pagas_prazo, SaldoRestante)
	select @Competencia, r.IDEmpresa, p.Nome, f.Matricula, p.CPF, r.parcela,CONVERT(varchar(2),(r.Parcela - (select COUNT(*) from AverbacaoParcela where IDAverbacao = r.idaverbacao and IDAverbacaoParcelaSituacao = 1 and Competencia <= @Competencia)))+'/'+CONVERT(varchar(2),r.prazo),
	(select SUM(valor) from AverbacaoParcela where IDAverbacao = r.idaverbacao and Competencia > @Competencia)	
	from #naorecuperavel r, #func f, Pessoa p
	where r.IDFuncionario = f.IDFuncionario and f.IDPessoa = p.IDPessoa
