...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[SP_IndiceNegocio]
...
BEGIN
	...
	delete from TmpColocacaoConvenio where competencia = @Competencia
	...
	-- colocacao no convenio --> producao
	insert into TmpColocacaoConvenio(competencia,IDEmpresa,tipo,IDServicoGrupo,Qtde,Total,Ano,Mes)
	select @Competencia,cm.idconsignataria,'Produzido',1,COUNT(*),SUM(cm.Prazo * ValorMovimento),convert(int,left(@competencia,4)),convert(int,right(@competencia,2))
	from conciliacaomovimento cm, AverbacaoParcela ap, Averbacao a
	where cm.Competencia = @Competencia and cm.IDAverbacaoParcela = ap.IDAverbacaoParcela and ap.IDAverbacao = a.IDAverbacao
	and a.CompetenciaInicial = @Competencia
	group by cm.IDConsignataria
	
	insert into TmpColocacaoConvenio(competencia,IDEmpresa,tipo,IDServicoGrupo,Qtde,Total,Ano,Mes)
	select @Competencia,cm.idconsignataria,'Carteira',1,COUNT(*),SUM(cm.Prazo * ValorMovimento),convert(int,left(@competencia,4)),convert(int,right(@competencia,2))
	from conciliacaomovimento cm
