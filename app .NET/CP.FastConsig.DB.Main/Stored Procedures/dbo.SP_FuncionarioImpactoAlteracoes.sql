...
-- =============================================
CREATE PROCEDURE [dbo].[SP_FuncionarioImpactoAlteracoes] 
...
BEGIN
    ...
    delete from TmpGrupoBoasNoticias where competencia = @Competencia
    ...
    select atual.idfuncionario, p.Cpf, atual.Matricula, p.Nome, m_atual.margemfolha as MargemAtual, m_antigo.margemfolha as MargemAnterior, m_atual.margemfolha-m_antigo.margemfolha as aumentomargem
    into #aumentomargem
	...
	-- 
	insert into TmpGrupoBoasNoticias(competencia,Grupo,Total,Possibilidade) 
    select @Competencia, 'Aumento de Margem', SUM(margemfolha), SUM(margemfolha*@prazomaximo)
	...
	if (@@ROWCOUNT > 0) begin
	  update TmpGrupoBoasNoticias set Total = isnull(Total,0) +
	  (select isnull(sum(margematual-margemanterior),0) from #aumentomargem),
	  Possibilidade = isnull(Possibilidade,0) + 
	  (select isnull(sum(margematual-margemanterior),0) from #aumentomargem) * @PrazoMaximo
	  where competencia = @Competencia and Grupo = 'Aumento de Margem'
	end
	else begin
      insert into TmpGrupoBoasNoticias(competencia,Grupo,Total,Possibilidade) 
      select @Competencia, 'Aumento de Margem', sum(margematual-margemanterior), SUM((margematual-margemanterior)*@prazomaximo)
      from #aumentomargem e, funcionariomargemcargadados f
      where f.competencia = @Competencia and f.idprodutogrupo = 1 and e.idfuncionario = f.idfuncionario 	
	end  
    
    select @idgrupo=id from TmpGrupoBoasNoticias g where g.competencia = @Competencia
    ...
    select @Competencia, null, @idgrupo, 'Novos Funcionários', COUNT(*), SUM(margemfolha), SUM(margemfolha*@prazomaximo), GETDATE() 
    from #entrou e, funcionariomargemcargadados f
    where f.competencia = @Competencia and f.idprodutogrupo = 1 and e.idfuncionario = f.idfuncionario 
    ...
    insert into TmpBoasNoticias(competencia, idconsignataria, IdGrupoBoaNoticia, Tipo, Qtde, ValorMargem, Possibilidade, createdon)
    select @Competencia, null, @idgrupo, 'Funcionários Antigos com aumento de margem', COUNT(*), SUM(aumentomargem), SUM(margemfolha*@prazomaximo), GETDATE() 
    from #aumentomargem e, funcionariomargemcargadados f
    where f.competencia = @Competencia and f.idprodutogrupo = 1 and e.idfuncionario = f.idfuncionario 
    ...
    insert into TmpBoasNoticias(competencia, idconsignataria, IdGrupoBoaNoticia, Tipo, Qtde, ValorMargem, Possibilidade, createdon)
    select @Competencia, m.idconsignataria, @idgrupo, 'Meus Clientes com aumento de margem', COUNT(*), SUM(aumentomargem), SUM(margemfolha*@prazomaximo), GETDATE() 
